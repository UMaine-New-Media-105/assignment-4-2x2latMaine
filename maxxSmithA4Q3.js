...
function draw() {
  ...
  for (let columnsDrawn = 0; columnsDrawn < pattern.columns; columnsDrawn++) {
    for (let rowsDrawn = 0; rowsDrawn < pattern.rows; rowsDrawn++) {
      ...
      //Randomize Color
      flower1.color = random(360);
      flower2.color = random(360);
      flower3.color = random(360);

      //Randomize Rotation
      flower1.rotation = random(360);
      flower2.rotation = random(360);
      flower3.rotation = random(360);

      //Randomize Size
      flower1.size = random(0.2, 0.3);
      flower2.size = random(0.15, 0.25);
      flower3.size = random(0.1, 0.2);
...
function drawFlower(flower) {
  ...
  //Flower Bulb
  if (flower.color >= 50 && flower.color <= 70) {
